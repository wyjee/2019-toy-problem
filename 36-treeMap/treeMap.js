...
Tree.prototype.map = function (callback, tree) {
  // return a new tree with the same structure as `this`, with values generated by the callback
  tree = tree || this;
  let newTree = new Tree(callback(tree.value))
  for(let i=0; tree.children.length; i++){
    newTree.children.push(tree.map(callback,tree.children[i]))
  }
  return newTree
};
...
Tree.prototype.mapInPlace = function (callback) {
  ...
  for (let i = 0; i < this.children.length; i++) {
    // Recursively call map on each child
